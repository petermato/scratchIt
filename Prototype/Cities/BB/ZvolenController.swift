...
class ZvolenController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Zvolen")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen11")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content11")
    }
    
    func checkForSegmentIndex(){
           if let value = defaults.value(forKey: "chosen11"){
       ...
       func checkForSavedText(){
           let text = defaults.value(forKey: "content11") as? String ?? "Povedz niečo o svojom výlete ... "
           textView.text = text
       }
