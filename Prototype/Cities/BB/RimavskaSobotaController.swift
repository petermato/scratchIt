//
//  RimavskaSobotaController.swift
//  Prototype
//
...
class RimavskaSobotaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Rimavsk%C3%A1_Sobota")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen9")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content9")
    }
    
    func checkForSegmentIndex(){
         if let value = defaults.value(forKey: "chosen9"){
     ...
     func checkForSavedText(){
         let text = defaults.value(forKey: "content9") as? String ?? "Povedz niečo o svojom výlete ... "
         textView.text = text
     }
