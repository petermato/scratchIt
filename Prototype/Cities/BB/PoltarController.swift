...
class PoltarController: UIViewController {
    ...
    @IBAction func OpenWeb(_ sender: UIButton) {
            showSafariVC(for: "https://sk.wikipedia.org/wiki/Polt%C3%A1r")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen7")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content7")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen7"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content7") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
