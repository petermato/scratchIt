...
class BreznoController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
            showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Brezno")
    }
    

    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosenThree")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "contentThree")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosenThree"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "contentThree") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
