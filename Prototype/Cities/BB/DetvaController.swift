...
class DetvaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
            showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Detva")
    }
    
    @IBAction func SegmentSelection(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen4")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
            defaults.set(textView.text!, forKey: "content4")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen4"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content4") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
