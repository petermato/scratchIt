...
class RevucaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Rev%C3%BAca_(mesto)")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen8")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content8")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen8"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content8") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
