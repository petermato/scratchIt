...
class KrupinaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
            showSafariVC(for: "https://sk.wikipedia.org/wiki/Krupina")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen5")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
            defaults.set(textView.text!, forKey: "content5")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen5"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content5") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
