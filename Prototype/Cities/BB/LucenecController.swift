...
class LucenecController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
            showSafariVC(for: "https://sk.wikipedia.org/wiki/Lu%C4%8Denec")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen6")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content6")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen6"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content6") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
