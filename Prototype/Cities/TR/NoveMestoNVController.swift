...
class NoveMestoNVController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Nov%C3%A9_Mesto_nad_V%C3%A1hom")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen49")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content49")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen49"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content49") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
