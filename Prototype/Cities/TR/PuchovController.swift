...
class PuchovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/P%C3%BAchov")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen53")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content53")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen53"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content53") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
