...
class PrievidzaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Prievidza")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen52")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content52")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen52"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content52") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
