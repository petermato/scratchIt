...
class PartizanskeController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Partiz%C3%A1nske")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen50")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content50")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen50"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content50") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
