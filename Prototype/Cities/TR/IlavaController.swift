...
class IlavaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Ilava")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen47")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content47")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen47"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content47") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
