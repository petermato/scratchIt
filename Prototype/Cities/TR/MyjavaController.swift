...
class MyjavaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Myjava_(mesto)")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen48")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content48")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen48"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content48") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
