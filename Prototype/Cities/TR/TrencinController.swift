...
class TrencinController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Tren%C4%8D%C3%ADn")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen54")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content54")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen54"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content54") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
