...
class BanovceNBController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/B%C3%A1novce_nad_Bebravou")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen46")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content46")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen46"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content46") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
