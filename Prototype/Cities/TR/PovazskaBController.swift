...
class PovazskaBController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Pova%C5%BEsk%C3%A1_Bystrica")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen51")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content51")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen51"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content51") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
