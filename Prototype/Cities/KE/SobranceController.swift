...
class SobranceController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Sobrance")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen23")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content23")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen23"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content23") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
