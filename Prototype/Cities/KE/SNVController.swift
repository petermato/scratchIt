...
class SNVController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Spi%C5%A1sk%C3%A1_Nov%C3%A1_Ves")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen24")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content24")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen24"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content24") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
