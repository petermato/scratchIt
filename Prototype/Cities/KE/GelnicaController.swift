...
class GelnicaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Gelnica")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen18")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content18")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen18"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content18") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
