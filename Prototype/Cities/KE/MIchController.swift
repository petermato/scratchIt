...
class MIchController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Michalovce")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen21")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content21")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen21"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content21") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
