...
class KEokolieController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Ko%C5%A1ice-okolie_(okres)")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen20")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content20")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen20"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content20") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
