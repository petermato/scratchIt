...
class TrebisovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Trebi%C5%A1ov")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen25")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content25")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen25"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content25") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
