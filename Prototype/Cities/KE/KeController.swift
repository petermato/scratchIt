...
class KeController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Košice")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen19")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content19")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen19"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content19") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
