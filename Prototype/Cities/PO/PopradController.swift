...
class PopradController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Poprad")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen38")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content38")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen38"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content38") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
