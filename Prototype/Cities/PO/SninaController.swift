...
class SninaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Snina")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen41")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content41")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen41"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content41") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
