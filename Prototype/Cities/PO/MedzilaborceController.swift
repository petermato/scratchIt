...
class MedzilaborceController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Medzilaborce")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen37")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content37")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen37"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content37") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
