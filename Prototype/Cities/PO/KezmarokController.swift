...
class KezmarokController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Ke%C5%BEmarok")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen35")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content35")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen35"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content35") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
