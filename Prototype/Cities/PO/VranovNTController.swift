...
class VranovNTController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Vranov_nad_Top%C4%BEou")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen45")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content45")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen45"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content45") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
