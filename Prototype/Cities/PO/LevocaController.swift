...
class LevocaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Levo%C4%8Da")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen36")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content36")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen36"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content36") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
