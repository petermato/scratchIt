...
class StaraLController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Star%C3%A1_%C4%BDubov%C5%88a")
    }
    
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen42")
        defaults.synchronize()
    }
    
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content42")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen42"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content42") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
