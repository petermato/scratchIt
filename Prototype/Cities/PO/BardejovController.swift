...
class BardejovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Bardejov")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen33")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content33")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen33"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content33") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
