...
class SabinovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Sabinov")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen40")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content40")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen40"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content40") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
