...
class StropkovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Stropkov")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen43")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content43")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen43"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content43") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
