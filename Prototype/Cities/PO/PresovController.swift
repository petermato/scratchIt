...
class PresovController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Pre%C5%A1ov")
    }
    
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen39")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content39")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen39"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content39") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
