...
class PezinokController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Pezinok")
    }
    
    @IBAction func segmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen16")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content16")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen16"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content16") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
