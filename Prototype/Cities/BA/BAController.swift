...
class BAController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Bratislava")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen14")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content14")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen14"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content14") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
