...
class MalackyController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Malacky")
    }
    
    @IBAction func segmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen15")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content15")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen15"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content15") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
