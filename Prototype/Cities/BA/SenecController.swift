...
class SenecController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Senec")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen17")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content17")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen17"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content17") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
