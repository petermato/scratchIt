...
class TopolcanyController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Topo%C4%BE%C4%8Dany")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen31")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content31")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen31"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content31") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
