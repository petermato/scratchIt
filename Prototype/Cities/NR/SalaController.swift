...
class SalaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/%C5%A0a%C4%BEa")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen30")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content30")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen30"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content30") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
