...
class KomarnoController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Kom%C3%A1rno")
    }
    
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen26")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content26")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen26"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content26") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
