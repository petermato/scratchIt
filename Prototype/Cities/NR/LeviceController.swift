...
class LeviceController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Levice_(Slovensko)")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen27")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content27")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen27"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content27") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
