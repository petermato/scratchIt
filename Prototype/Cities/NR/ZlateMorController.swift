...
class ZlateMorController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Zlat%C3%A9_Moravce")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen32")
        defaults.synchronize()
    }
    
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content32")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen32"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content32") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
