...
class NoveZamkyController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Nov%C3%A9_Z%C3%A1mky")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen29")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content29")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen29"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content29") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
