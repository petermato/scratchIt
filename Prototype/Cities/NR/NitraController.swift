...
class NitraController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.m.wikipedia.org/wiki/Nitra")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen28")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content28")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen28"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content28") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
