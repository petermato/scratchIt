//
//  DunajskaStredaController.swift
//  Prototype
//
...
class DunajskaStredaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Dunajsk%C3%A1_Streda")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen55")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content55")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen55"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content55") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
