...
class SkalicaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Skalica_(mesto)")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen60")
        defaults.synchronize()
        
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content60")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen60"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content60") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
