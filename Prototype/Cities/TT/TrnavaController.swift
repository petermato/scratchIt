...
class TrnavaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Trnava")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen61")
        defaults.synchronize()
        
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content61")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen61"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content61") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
