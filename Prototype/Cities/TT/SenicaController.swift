...
class SenicaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Senica")
        
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen59")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content59")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen59"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content59") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
