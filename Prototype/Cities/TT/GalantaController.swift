...
class GalantaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Galanta")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen56")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content56")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen56"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content56") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
