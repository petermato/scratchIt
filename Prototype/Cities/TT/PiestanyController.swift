...
class PiestanyController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Pie%C5%A1%C5%A5any")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen58")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content58")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen58"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content58") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
