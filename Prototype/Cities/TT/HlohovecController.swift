...
class HlohovecController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Hlohovec")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen57")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content57")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen57"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content57") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
