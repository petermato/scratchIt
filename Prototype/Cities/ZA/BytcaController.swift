...
class BytcaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "")
        //showSafariVC(for: "https://sk.wikipedia.org/wiki/Byt%C4%8Da")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen62")
           defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content62")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen62"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content62") as? String ?? "Povedz niečo o svojom výlete ... "
            textView.text = text
            }
