...
class TvrdosinController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Tvrdo%C5%A1%C3%ADn")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen71")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content71")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen71"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content71") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
