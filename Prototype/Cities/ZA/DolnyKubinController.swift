...
class DolnyKubinController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Doln%C3%BD_Kub%C3%ADn")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen64")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content64")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen64"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content64") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
