//
//  KysuckeNoveMestoController.swift
//  Prototype
//
...
class KysuckeNoveMestoController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Kysuck%C3%A9_Nov%C3%A9_Mesto")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen65")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content65")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen65"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content65") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
