...
class RuzomberokController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Ru%C5%BEomberok")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen69")
        defaults.synchronize()
    }
    
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content69")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen69"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content69") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
