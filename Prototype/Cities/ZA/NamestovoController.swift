...
class NamestovoController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/N%C3%A1mestovo")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen68")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content68")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen68"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content68") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
