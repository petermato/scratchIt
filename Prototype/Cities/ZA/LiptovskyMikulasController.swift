//
//  LiptovskyMikulasController.swift
//  Prototype
//
...
class LiptovskyMikulasController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Liptovsk%C3%BD_Mikul%C3%A1%C5%A1")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen66")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content66")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen66"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content66") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
