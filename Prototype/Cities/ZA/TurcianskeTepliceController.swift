//
//  TurcianskeTepliceController.swift
//  Prototype
//
...
class TurcianskeTepliceController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Tur%C4%8Dianske_Teplice")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen70")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content70")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen70"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content70") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
