...
class ZilinaController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/%C5%BDilina")
    }
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen72")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content72")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen72"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content72") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
