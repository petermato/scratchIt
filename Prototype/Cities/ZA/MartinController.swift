...
class MartinController: UIViewController {
    ...
    @IBAction func WebOpen(_ sender: UIButton) {
        showSafariVC(for: "https://sk.wikipedia.org/wiki/Martin_(mesto_na_Slovensku)")
    }
    
    
    @IBAction func SegmentButton(_ sender: UISegmentedControl) {
        defaults.set(sender.selectedSegmentIndex, forKey: "chosen67")
        defaults.synchronize()
    }
    
    @IBAction func SaveButton(_ sender: UIButton) {
        defaults.set(textView.text!, forKey: "content67")
    }
    
    func checkForSegmentIndex(){
        if let value = defaults.value(forKey: "chosen67"){
    ...
    func checkForSavedText(){
        let text = defaults.value(forKey: "content67") as? String ?? "Povedz niečo o svojom výlete ... "
        textView.text = text
    }
